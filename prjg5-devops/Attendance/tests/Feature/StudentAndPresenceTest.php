...
class StudentAndPresenceTest extends TestCase
{
    ...
    public function test_set_presence_student(){
        ...
        $data = [['student_id' => $student->id, "date" => date('Y-m-d'), "is_present" => true]];
        ...
        $this->assertDatabaseHas('student_presences', ['student_id' => $student->id, "date" => date('Y-m-d'), "is_present" => true]);

        $data = [['student_id' => $student->id, "date" => date('Y-m-d'), "is_present" => false]];
        $response = $this->postJson('/presence/take', $data);
        $response
            ->assertStatus(201)
            ->assertJson([
        ...
        $this->assertDatabaseHas('student_presences', ['student_id' => $student->id, "date" => date('Y-m-d'), "is_present" => false]);
    }
